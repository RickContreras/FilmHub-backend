...
if [ -z "$AZ_RESOURCE_GROUP" ] || [ -z "$AZ_DATABASE_SERVER_NAME" ] || [ -z "$AZ_DATABASE_NAME" ] || [ -z "$AZ_LOCATION" ] || [ -z "$AZ_SQL_SERVER_USERNAME" ] || [ -z "$AZ_SQL_SERVER_PASSWORD" ] || [ -z "$AZ_LOCAL_IP_ADDRESS" ]; then
...
az sql server firewall-rule create \
    --resource-group $AZ_RESOURCE_GROUP \
    --name ${AZ_DATABASE_SERVER_NAME}-allow-local-ip \
